...
#ifdef DEBUG
	uprintf(UART0, "RF: ret:%d, st: %d. : to : %d : len : %d  : data : %c\n", ret, status, data[1], data[0], data[2]);
#endif
	if(data[1] != 0)
	{
		...
		switch (data[2]) {
			case 'T':
				{
					...
					if (ret != 0) {
						send_uint16_on_RF(RF_SEND_ADDR, 'T', val);
					} else {
						send_error_on_rf(RF_SEND_ADDR, 'T');
					}
	#ifdef DEBUG
					if (ret != 0) {
			...
			case 'V':
				{
					...
					if (ret != 0) {
						send_uint16_on_RF(RF_SEND_ADDR, 'V', val);
					} else {
						send_error_on_rf(RF_SEND_ADDR, 'V');
					}
	#ifdef DEBUG
					if (ret >= 0) {
			...
			default: /* This is a color code, three ascii values */
				{
					timer_set_match(LPC_TIMER_32B1, 0, ((data[2] - '0') * 20));
					timer_set_match(LPC_TIMER_32B1, 1, ((data[3] - '0') * 20));
					timer_set_match(LPC_TIMER_32B1, 2, ((data[4] - '0') * 20));
				}
				break;
		}
	}
...
void send_uart_to_rf(void)
{
	...
	cc_tx_data[1] = RF_SEND_ADDR; /* Broadcast */
	/* Send */
	if (cc1101_tx_fifo_state() != 0) {
		cc1101_flush_tx_fifo();
	}
