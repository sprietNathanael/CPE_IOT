...
void temp_read(int* deci_degrees, uint16_t* raw, int* ret)
{
...
void temp_display(int uart_num, int deci_degrees, int raw, int ret)
{
...
int main(void)
{
	...
	while (1) {
		temp_read(&deci_degrees, &raw, &ret);
		degrees = deci_degrees/10;
		if(degrees < 26)
		{
		...
		msleep(250);
		//chenillard(250);
		temp_display(UART0, deci_degrees, raw, ret);
	}
	return 0;
}
